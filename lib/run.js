...
const initializeDatabase = co.wrap(function *(newDatabaseName, tableType) {
	try {
		const dbClient = yield dbUtils.createClient(dbUtils.createConnectionUrl(R.merge(R.pick(['host', 'user', 'password'], config.connectionParams),
...
const main = co.wrap(function * (newDatabaseName, tableType) {
