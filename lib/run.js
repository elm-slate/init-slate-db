...
const createDestinationTable = co.wrap(function *(connectionString) {
	try {
		...
		var sqlStatement =
			...
			`CONSTRAINT events_pkey PRIMARY KEY (id)) ` +
			`WITH (OIDS=FALSE)`;
		var result = yield dbUtils.executeSQLStatement(dbClient, sqlStatement);
		logger.info({result: result}, `events table created in database "${dbClientDatabase}"`);
		sqlStatement = `CREATE INDEX events_event_name on events ((event #>> '{name}'))`;
		result = yield dbUtils.executeSQLStatement(dbClient, sqlStatement);
		logger.info({result: result}, `events_event_name index created in database "${dbClientDatabase}"`);
		sqlStatement = `CREATE INDEX events_eventtimestamp on events (eventTimestamp)`;
		result = yield dbUtils.executeSQLStatement(dbClient, sqlStatement);
		logger.info({result: result}, `events_eventtimestamp index created in database "${dbClientDatabase}"`);
	}
	catch(err) {
...
const main = co.wrap(function * (newDatabase, tableType) {
	...
	const masterConnectionString = dbUtils.createConnectionUrl(R.merge(R.pick(['host', 'user', 'password', 'connectTimeout'], config.connectionParams), {databaseName: 'postgres'}));
	...
	if (result) {
		...
		const newDbConnectionString = dbUtils.createConnectionUrl(R.merge(R.pick(['host', 'user', 'password', 'connectTimeout'], config.connectionParams), {databaseName: newDatabase}));
		if (tableType === 'source') {
