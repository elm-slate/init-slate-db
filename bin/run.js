...
const createEventsTable = co.wrap(function *(dbClient, dbClientDatabase) {
	...
	yield dbUtils.executeSQLStatement(dbClient, `CREATE INDEX events_event_entityname on events ((event #>> '{entityName}'))`);
	logger.info(`events_event_entityname index created in database "${dbClientDatabase}"`);
	...
	yield dbUtils.executeSQLStatement(dbClient, `CREATE INDEX events_event_metadata_command on events ((event #>> '{metadata, command}'))`);
	logger.info(`events_event_metadata_command index created in database "${dbClientDatabase}"`);
